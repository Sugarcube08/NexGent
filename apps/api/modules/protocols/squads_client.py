...
class SquadsClient:
    ...
    async def deploy_agent_treasury(
        self, agent_id: str, creator_wallet: str
    ) -> Optional[str]:
        """
        ...
        logger.info(
            f"VACN_SQUADS: Initializing on-chain treasury for {agent_id} at {multisig_pda}"
        )
        ...
        try:
            ...
            async with AsyncClient(SOLANA_RPC_URL) as client:
                ...
                if acc_info.value:
                    logger.info(
                        f"VACN_SQUADS: Treasury already exists at {multisig_pda}"
                    )
                ...
                logger.info(
                    f"VACN_SQUADS: Created multisig treasury {multisig_pda}. Tx: {resp.value}"
                )
    ...
    async def get_next_transaction_index(
        self, client: AsyncClient, multisig_pda: Pubkey
    ) -> int:
        acc_info = await client.get_account_info(multisig_pda)
    ...
    async def create_withdrawal_proposal(
        self,
        ...
        """
        logger.info(
            f"VACN_SQUADS: Creating withdrawal proposal for {amount_sol} SOL from {multisig_pda_str} to {recipient_str}"
        )
        ...
        try:
            ...
            async with AsyncClient(SOLANA_RPC_URL) as client:
                ...
                data_vt = (
                    disc_vt + struct.pack("<B", 0) + struct.pack("<B", 0)
                )  # vaultIndex=0, ephemeralSigners=0
                data_vt += (
                    struct.pack("<I", len(inner_msg_bytes)) + inner_msg_bytes
                ...
                ix_vt = Instruction(
                    ...
                    accounts=[
                        AccountMeta(
                            pubkey=multisig_pda, is_signer=False, is_writable=True
                        ),
                        AccountMeta(
                            pubkey=transaction_pda, is_signer=False, is_writable=True
                        ),
                        AccountMeta(
                            pubkey=platform_keypair.pubkey(),
                            is_signer=True,
                ...
                data_pc = (
                    disc_pc + struct.pack("<Q", tx_index) + b"\x00"
                )  # tx_index, draft=false
                ix_pc = Instruction(
                    program_id=self.program_id,
                    data=data_pc,
                    accounts=[
                        AccountMeta(
                            pubkey=multisig_pda, is_signer=False, is_writable=False
                        ),
                        AccountMeta(
                            pubkey=proposal_pda, is_signer=False, is_writable=True
                        ),
                        AccountMeta(
                            pubkey=platform_keypair.pubkey(),
                            is_signer=True,
                ...
                ix_pa = Instruction(
                    ...
                    accounts=[
                        ...
                        AccountMeta(
                            pubkey=member_pda, is_signer=False, is_writable=False
                        ),
                        AccountMeta(
                            pubkey=proposal_pda, is_signer=False, is_writable=True
                        ),
                ...
                resp = await client.send_transaction(tx)
                logger.info(
                    f"VACN_SQUADS: Proposal created and active on {multisig_pda}. Tx: {resp.value}"
                )
    ...
    async def sign_m2m_escrow(
        self, hiring_treasury_pda: str, hired_agent_id: str, amount: float
    ) -> bool:
        """
        ...
        """
        logger.info(
            f"VACN_SQUADS: Treasury {hiring_treasury_pda} proposing {amount} SOL funding for {hired_agent_id}"
        )

        try:
            # We derive the escrow PDA that needs to be funded
            Pubkey.from_string("SHoujikiEscrow11111111111111111111111111111")
            ...
            create_seed = hashlib.sha256(
                f"squads_create_v1:{hired_agent_id}".encode()
            ).digest()
            create_keypair = Keypair.from_seed(create_seed)
