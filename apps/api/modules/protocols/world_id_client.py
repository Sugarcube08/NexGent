...
class WorldIDClient:
    ...
    async def verify_human_creator(self, wallet_address: str, verification_data: dict) -> str:
        """
        ...
        logger.info(f"WORLD_ID: Verifying ZKP for creator {wallet_address} with nullifier {nullifier_hash}")
        ...
        try:
            ...
            def fetch():
                return requests.post(f"{self.api_url}/{self.app_id}", json=payload, timeout=10)
            
            resp = await asyncio.to_thread(fetch)
            
            if resp.status_code != 200:
                # If it's a 400 error because of staging, we log but still allow it
                if "invalid_proof" in resp.text and self.app_id == "app_agentos_staging":
                    logger.warning(f"WORLD_ID: Development environment bypass for invalid proof: {resp.text}")
                else:
        ...
        logger.info(f"WORLD_ID: Successfully verified ZKP for {wallet_address}. Personhood confirmed.")
