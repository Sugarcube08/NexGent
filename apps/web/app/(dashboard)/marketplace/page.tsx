  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-1000">
      ...
      <div className="relative overflow-hidden rounded-[32px] border border-white/5 bg-white/[0.02] p-12">
        <div className="absolute top-0 right-0 w-1/2 h-full bg-gradient-to-l from-cyber-cyan/5 to-transparent pointer-events-none" />
        <div className="relative z-10 max-w-2xl space-y-6">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyber-cyan/10 border border-cyber-cyan/20 text-cyber-cyan text-[10px] font-black uppercase tracking-widest">
            <Sparkles size={12} />
            Live_Market_Active
          </div>
          <h1 className="text-5xl font-black text-white tracking-tighter leading-tight uppercase italic">
            Neural <span className="text-cyber-cyan">Labor</span> <br />Exchange
          </h1>
          ...
          <div className="flex items-center gap-4 pt-4">
            ...
            <div className="flex items-center gap-6 px-6 border-l border-white/10">
              ...
              <div>
                ...
                <p className="text-lg font-mono font-black text-cyber-cyan">14.8k</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row items-center gap-6">
        <div className="flex-1 w-full relative group">
          ...
          <input 
            ...
            className="w-full bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl h-14 pl-12 pr-6 text-sm font-medium text-zinc-200 focus:outline-none focus:border-cyber-cyan/30 focus:ring-1 focus:ring-cyber-cyan/20 transition-all shadow-glass-inner"
          />
        </div>
      ...
      {filteredAgents.length === 0 && (
        <div className="py-20 text-center space-y-4 border border-dashed border-white/10 rounded-[32px]">
          ...
          <Button variant="ghost" onClick={() => setSearch('')} className="text-[10px] font-black uppercase tracking-widest text-cyber-cyan">Reset_Registry_View</Button>
        </div>
      )}
    </div>
  );
